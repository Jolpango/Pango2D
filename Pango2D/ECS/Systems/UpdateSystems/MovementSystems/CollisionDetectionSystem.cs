...
namespace Pango2D.ECS.Systems.UpdateSystems.MovementSystems
{
    internal class CollisionDetectionSystem : IPostUpdateSystem
    {
        ...
        {
            foreach(var(entity, events) in World.Query<CollisionEvents>())
            {
                events.Events.Clear();
            }
        ...
        {
            AddCollisionEvent(entityA, entityB, normal, penetrationDepth);
            AddCollisionEvent(entityB, entityA, -normal, penetrationDepth);
        }

        private void AddCollisionEvent(Entity entity, Entity other, Vector2 normal, float penetrationDepth)
        {
            if (!World.TryGetComponent<CollisionEvents>(entity, out var events))
            {
                events = new CollisionEvents();
                World.AddComponent(entity, events);
            }
            events.Events.Add(new CollisionEvent { Other = other, Normal = normal, PenetrationDepth = penetrationDepth });
        }
