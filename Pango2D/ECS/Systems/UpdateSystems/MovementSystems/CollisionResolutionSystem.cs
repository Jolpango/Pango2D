...
namespace Pango2D.ECS.Systems.UpdateSystems.MovementSystems
{
    internal class CollisionResolutionSystem : IPostUpdateSystem
    {
        ...
        {
            foreach (var (entity, collisionEvent) in World.Query<CollisionEvents>())
            {
                foreach (var collision in collisionEvent.Events)
                {
                    if (entity.Id < collision.Other.Id)
                    {
                        ResolveCollision(entity, collision.Other, collision.Normal, collision.PenetrationDepth);
                    }
                }
            }
        }
                                      ...
                                      Entity staticEntity, Collider staticCollider, Vector2 normal, float penetrationDepth)
        {
                ...
                World.TryGetComponent<Acceleration>(dynamicEntity, out var acceleration))
            {
                ...
                {
                    transform.Position += normal * (int)(penetrationDepth - 1);
                }
            }
        }
